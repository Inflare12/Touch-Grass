...
/** Conservative temporal verifier. The local vision implementation is heuristic, so verification requires multiple signals. */
class ContactVerifier(requiredHoldSeconds: Int = 3) {
    private var requiredHoldSeconds: Int = requiredHoldSeconds.coerceIn(1, 30)
    ...
    fun setRequiredHoldSeconds(seconds: Int) {
        requiredHoldSeconds = seconds.coerceIn(1, 30)
    }
    ...
    fun processFrame(
        ...
        val isContact = isGrassPassed && isHandPassed && hasGroundEvidence &&
            handResult.handCentroidY > 0.58f && handResult.isMovingDownTowardsGrass
        ...
        fun holdingState(): VerificationState {
            ...
            val total = requiredHoldSeconds * 1000L
            if (elapsed >= total) {
                val duration = ((System.currentTimeMillis() - verificationStartTimeMillis) / 1000L).toInt().coerceAtLeast(1)
                return VerificationState.Verified(duration)
            }
            val remaining = kotlin.math.ceil((total - elapsed) / 1000.0).toInt().coerceAtLeast(1)
            return VerificationState.Holding(remaining, (elapsed.toFloat() / total).coerceIn(0f, 1f))
        }
        ...
        when (currentState) {
            ...
            VerificationState.HandDetected -> when {
                confirmedContact -> { beginHold(); currentState = VerificationState.ContactDetected }
